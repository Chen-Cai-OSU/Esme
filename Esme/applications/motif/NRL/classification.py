...
def svc_classification(X_train, X_test, y_train_, y_train, y_test_, y_test, grid_search, multi_label_binarizer, test_kernel):
    ...
    if test_kernel == 'eigenpro':
        ...
        n_train = min(int(len(X_train)*0.5), 1000) # number of datapoints used for hyper-parameter search
        ...
        res, prob = model.fit(X_train, y_train_eig, X_train, y_train_eig, epochs=[1, 2, 5, 10, 20], mem_gb=12, print_flag=print_flag)
