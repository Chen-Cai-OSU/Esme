...
def sw(dgms1, dgms2, parallel_flag=False, kernel_type='sw', **kwargs):
    ...
    if parallel_flag==False:
        if kernel_type=='sw':
            tda_kernel = tda.SlicedWassersteinKernel(num_directions=kwargs['n_d'], bandwidth=kwargs['bw'])
...
def sw_parallel(dgms1, dgms2,  kernel_type='sw', parallel_flag=True, n_directions=10, granularity=25, **kwargs):
    ...
    :param n_directions: n_dictions to approximate sw kernel
...
if __name__ == '__main__':
    ...
    # dgms2 = random_dgms(3)
    print(sw_parallel(dgms1, dgms1, parallel_flag=False, kernel_type='sw', n_directions=10, bandwidth=1.0, K=1, p=1))
