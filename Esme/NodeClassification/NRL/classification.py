...
from NodeClassification.NRL.eigenpro import init_train_Gaussian, line_search, Gaussian, FKR_EigenPro, get_n_class, encoder_
...
def logistic_regression_classification(X_train, X_test, y_train_, y_train, y_test_, y_test, grid_search, multi_label_binarizer):
    lf_classifer = OneVsRestClassifier(LogisticRegression(), n_jobs=-1)
    if grid_search:
        parameters = {
...
def svc_classification(X_train, X_test, y_train_, y_train, y_test_, y_test, grid_search, multi_label_binarizer, test_kernel):
    ...
    if test_kernel == 'eigenpro':
        ...
        try:
            # best_val, best_param = line_search(eval_f, 1, 100, print_flag=print_flag)  # search kernel bandwidth in [1, 50]
            best_param = 1
        ...
        res, prob = model.fit(X_train[:n_train], y_train_eig[:n_train], X_val, y_val, epochs=[1, 10, 30, 50, 100], mem_gb=12, print_flag=print_flag)
